...
def create_dfs(path, commentstring=None, colnames=None, low_limit=-float('inf'), high_limit=float('inf')):
    dfs = {}
...
def create_df(path, commentstring=None, colnames=None, low_limit=-float('inf'), high_limit=float('inf')):
...
def create_range(df_dict, colname, l_limit, h_limit, step):
    index = np.arange(l_limit, h_limit, step)
    mean_df = pd.DataFrame(index=index)
    for k,v in df_dict.iteritems():
        mean_df.insert(0, k, get(v[colname], l_limit, h_limit, step))
    return mean_df
...
class RedPanda(object):
    ...
    def create_range(self, df_dict, colname, l_limit, h_limit, step):
        ...
        mean_df = pd.DataFrame(index=index)
        for k,v in df_dict.iteritems():
            mean_df.insert(0, k, RedPanda.get(v[colname], l_limit, h_limit, step))
        return mean_df
    ...
    def meq_relfreq(self, df_dict, colname, l_limit, h_limit, step, numbins=10):
        range_df = RedPanda.create_range(df_dict, colname, l_limit, h_limit, step)
        rangeX = np.arange(l_limit, h_limit, step)
        X = np.zeros((len(rangeX),numbins))
        for x in range(len(rangeX)):
            X[x] = rangeX[x]
